...
from image_coregistration.utils.coreg_utils import ImageRotate
from image_coregistration.utils.rotation_coreg_phase_corr import (
    PhaseCorrDiffEvolutionRotationPolar,
)
from image_coregistration.utils.rotation_coreg_cross_corr import (
    CrossCorrDiffEvolutionRotationPolar,
)
from image_coregistration.utils.rotation_coreg_mutual_info import (
...
for i in range(Ntrials):
    ...
    rot_angle = PhaseCorrDiffEvolutionRotationPolar(
        image_ref.copy(), image_rotated.copy(), bounds, Niter
    )
    end = time.time()
    ...
    bounds = [(-1.5, 1.5)]
    # bounds for sub-pixel level coregestration
    rot_angle = CrossCorrDiffEvolutionRotationPolar(
        image_ref.copy(), image_rotated.copy(), bounds, Niter
    )
    end = time.time()
    ...
    # Mutual information methods
    start = time.time()
    bounds = [(limit_left * 2, limit_right * 2)]
    # bounds for sub-pixel level coregestration
    rot_angle = MutualInfoDiffEvolutionRotationCartesian(
        image_ref.copy(), image_rotated.copy(), bounds, Niter
    )
    end = time.time()
...
plt.subplot(1, 3, 1)
plt.hist(
    method1[0], bins, color="#0504aa", alpha=0.5, facecolor="blue", weights=weights
)
...
plt.subplot(1, 3, 2)
plt.hist(
    method2[0], bins, color="#0504aa", alpha=0.5, facecolor="blue", weights=weights
)
plt.grid(axis="y", alpha=0.75)
plt.xlabel("Error [degree]")
plt.ylabel("Probability")
...
plt.subplot(1, 3, 3)
plt.hist(
    method3[0], bins, color="#0504aa", alpha=0.5, facecolor="blue", weights=weights
)
plt.grid(axis="y", alpha=0.75)
plt.xlabel("Error [degree]")
plt.ylabel("Probability")
