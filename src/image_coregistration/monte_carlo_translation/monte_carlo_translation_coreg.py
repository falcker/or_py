...
from image_coregistration.utils.coreg_utils import ImageTranslate
from image_coregistration.utils.translation_coreg_phase_corr import (
    PhaseCorrDiamondSearchTranslation,
    PhaseCorrFullSearchTranslation,
    PhaseCorrDiffEvolutionTranslation,
)
from image_coregistration.utils.translation_coreg_cross_corr import (
    CrossCorrDiamondSearchTranslation,
    CrossCorrFullSearchTranslation,
    CrossCorrDiffEvolutionTranslation,
)
from image_coregistration.utils.translation_coreg_mutual_info import (
...
for i in range(Ntrials):
    ...
    # Phase correlation methods
    start = time.time()
    shift_row, shift_col = PhaseCorrDiamondSearchTranslation(
        image_ref.copy(), image_shifted.copy(), Niter
    )
    end = time.time()
    ...
    start = time.time()
    shift_row, shift_col = PhaseCorrFullSearchTranslation(
        image_ref.copy(), image_shifted.copy(), Niter
    )
    end = time.time()
    ...
    start = time.time()
    bounds = [(-2, 2), (-2, 2)]
    # bounds for sub-pixel level coregestration
    shift_row, shift_col = PhaseCorrDiffEvolutionTranslation(
        image_ref.copy(), image_shifted.copy(), bounds, Niter
    )
    end = time.time()
    ...
    # Cross correlation methods
    start = time.time()
    shift_row, shift_col = CrossCorrDiamondSearchTranslation(
        image_ref.copy(), image_shifted.copy(), Niter
    )
    end = time.time()
    ...
    start = time.time()
    shift_row, shift_col = CrossCorrFullSearchTranslation(
        image_ref.copy(), image_shifted.copy(), Niter
    )
    end = time.time()
    ...
    bounds = [(-2, 2), (-2, 2)]
    # bounds for sub-pixel level coregestration
    shift_row, shift_col = CrossCorrDiffEvolutionTranslation(
        image_ref.copy(), image_shifted.copy(), bounds, Niter
    )
    end = time.time()
    ...
    # Mutual information methods
    start = time.time()
    bounds = [(limit_left * 2, limit_right * 2), (limit_left * 2, limit_right * 2)]
    # bounds for sub-pixel level coregestration
    shift_row, shift_col = MutualInfoDiffEvolutionTranslation(
        image_ref.copy(), image_shifted.copy(), bounds, Niter
    )
    end = time.time()
...
plt.text(
    -0.99,
    0.16,
    r"rows $ {} \pm {}$".format(round(mu_row, 2), round(sigma_row, 2)),
    fontsize=kfontsize,
)
plt.text(
    -0.99,
    0.15,
    r"cols $ {} \pm {}$".format(round(mu_col, 2), round(sigma_col, 2)),
    fontsize=kfontsize,
)
plt.text(
    -0.99,
    0.14,
    r"avg trial dur ${}ms$".format(round(time_array_avg[0], 2), fontsize=kfontsize),
)
...
plt.text(
    -0.99,
    0.14,
    r"avg trial dur ${}ms$".format(round(time_array_avg[1], 2), fontsize=kfontsize),
)
plt.subplots_adjust(hspace=0.001)
plt.tight_layout()
...
plt.text(
    -0.99,
    0.14,
    r"avg trial dur ${}ms$".format(round(time_array_avg[2], 2), fontsize=kfontsize),
)
plt.subplots_adjust(hspace=0.001)
plt.tight_layout()
...
plt.text(
    -0.99,
    0.14,
    r"avg trial dur ${}ms$".format(round(time_array_avg[3], 2), fontsize=kfontsize),
)
plt.subplots_adjust(hspace=0.001)
...
plt.text(
    -0.99,
    0.14,
    r"avg trial dur ${}ms$".format(round(time_array_avg[4], 2), fontsize=kfontsize),
)
plt.subplots_adjust(hspace=0.001)
...
plt.text(
    -0.99,
    0.14,
    r"avg trial dur ${}ms$".format(round(time_array_avg[5], 2), fontsize=kfontsize),
)
plt.subplots_adjust(hspace=0.001)
plt.tight_layout()
...
plt.text(
    -0.99,
    0.14,
    r"avg trial dur ${}ms$".format(round(time_array_avg[6], 2), fontsize=kfontsize),
)
plt.subplots_adjust(hspace=0.001)
plt.tight_layout()
