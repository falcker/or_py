...
import numpy as np
from image_coregistration.utils.coreg_utils import (
    ImageTranslate,
    _NormalizeImage,
    _ApplyHighPassFilter,
    _MseMetric,
    _ApplyHannWindow,
    _NeighborhoodDiamondSearch,
    _NeighborhoodFullSearch,
    _DiffEvolutionTranslation,
)
...
def _PhaseCorr(image_ref, image_shifted):
    ...
    if ind[0] > Nrows / 2:
        shift_row = ind[0] - Nrows
        # to correctly handle negative shift
    else:
        shift_row = ind[0]

    if ind[1] > Ncols / 2:
        shift_col = ind[1] - Ncols
        # to correctly handle negative shift
    else:
        shift_col = ind[1]
...
def PhaseCorrDiamondSearchTranslation(image_ref, image_shifted, Niter):
    ...
    delta_row, delta_col = _NeighborhoodDiamondSearch(
        image_ref, image_shifted, 0, 0, Niter
    )
...
def PhaseCorrFullSearchTranslation(image_ref, image_shifted, Niter):
    ...
    # sub-pixel level coregestration
    delta_row, delta_col = _NeighborhoodFullSearch(
        image_ref, image_shifted, 0, 0, Niter
    )
    return shift_row + delta_row, shift_col + delta_col
...
def PhaseCorrDiffEvolutionTranslation(image_ref, image_shifted, bounds, Niter):
    ...
    # sub-pixel level coregestration
    obj_func = lambda shift: _MseMetric(
        image_ref, ImageTranslate(image_shifted, -shift[0], -shift[1])
    )
